...
namespace DataEditorX.Core.Mse
{
	...
	public class MseMaker
	{
		...
		public const string TAG_OCGDATE = "ocgdate";
		public const string TAG_TCGDATE = "tcgdate";
...
//			MessageBox.Show(""+cfg.replaces.Keys[0]+"/"+cfg.replaces[cfg.replaces.Keys[0]]);
		...
		public Card ReadCard(string content, out string img)
		{
			...
			//Zeak6464
			int.TryParse(GetValue(content, TAG_SCRIPT), out c.script);
			int.TryParse(GetValue(content, TAG_SUPPORT), out c.support);
			long.TryParse(GetValue(content, TAG_OCGDATE), out c.ocgdate);
			int.TryParse(GetValue(content, TAG_TCGDATE), out c.tcgdate);
			//ATK
