...
namespace DataEditorX.Core
{
	...
	public static class DataBase
	{
		...
		static DataBase()
		{
			...
			st.Append("level integer,race integer,attribute integer,category integer,genre integer,");
			st.Append("script blob,support integer,ocgdate integer,tcgdate integer)");
		...
		static Card ReadCard(SQLiteDataReader reader,bool reNewLine)
		{
			...
			c.genre = reader.GetInt64(reader.GetOrdinal("genre"));
			c.support = reader.GetInt32(reader.GetOrdinal("support"));
			c.ocgdate = reader.GetInt32(reader.GetOrdinal("ocgdate"));
			c.tcgdate = reader.GetInt32(reader.GetOrdinal("tcgdate"));
		...
		public static string GetSelectSQL(Card c)
		{
			...
			if(c.genre>0)
				sb.Append(" and datas.genre = & "+toInt(c.genre)+" = "+toInt(c.genre));
			if(c.script>0)
				sb.Append(" and datas.script = "+c.script.ToString());
			if(c.support>0)
				sb.Append(" and datas.support = "+c.support.ToString());
			if(c.ocgdate>0)
				sb.Append(" and datas.ocgdate = "+c.ocgdate.ToString());
			if(c.tcgdate>0)
				sb.Append(" and datas.tcgdate = "+c.tcgdate.ToString());
			if(c.atk==-1)
		...
		public static string GetInsertSQL(Card c, bool ignore,bool hex= false)
		{
			...
			if(hex){
				...
				st.Append("0x"+c.genre.ToString("x")); st.Append(")");
			}else{
				...
				st.Append(c.genre.ToString()); st.Append(")");
			}
			if(ignore)
		...
		public static string GetUpdateSQL(Card c)
		{
			...
			st.Append(",genre="); st.Append(c.genre.ToString());
